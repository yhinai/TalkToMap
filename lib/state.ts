...
export type Template = 'business-analyst';

const toolsets: Record<Template, FunctionCall[]> = {
  'business-analyst': itineraryPlannerTools,
};
...
const systemPrompts: Record<Template, string> = {
  'business-analyst': SYSTEM_INSTRUCTIONS,
};
...
export const useSettings = create<{
  ...
  systemPrompt: systemPrompts['business-analyst'],
...
export const useDashboardStore = create<{
  ...
  charts: ChartConfig[];
  isVisible: boolean;
  setData: (data: { metrics?: Metric[]; charts?: ChartConfig[] }) => void;
  clearData: () => void;
}>(set => ({
  metrics: [],
  charts: [],
  ...
  setData: ({ metrics = [], charts = [] }) =>
    set({
      metrics,
      charts,
      isVisible: metrics.length > 0 || charts.length > 0,
    }),
  clearData: () => set({ metrics: [], charts: [], isVisible: false }),
}));
