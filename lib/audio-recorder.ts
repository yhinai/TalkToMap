...
export class AudioRecorder {
  ...
  async start() {
    ...
    this.starting = new Promise(async (resolve, reject) => {
      ...
      // Create an AudioContext with the device's default sample rate to avoid errors.
      ...
      // Pass source and target sample rates to the worklet for resampling.
      this.recordingWorklet = new AudioWorkletNode(
        this.audioContext,
        workletName,
        {
          processorOptions: {
            targetSampleRate: this.sampleRate, // Target is 16000Hz for the API
            sourceSampleRate: this.audioContext.sampleRate, // Source is the hardware rate
          },
        }
      );
