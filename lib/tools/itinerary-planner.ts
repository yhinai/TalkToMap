...
export const itineraryPlannerTools: FunctionCall[] = [
  {
    ...
    1.  **For Business Analysis:** Find and summarize information about business environments, such as restaurants, museums, or parks in a city. Use a straightforward query to get factual summaries of top results, which you can then analyze.
        -   **Example Query:** "coffee shops in Seattle" or "top brands in Brooklyn".
  ...
  },
  {
    ...
    description:
      'Displays business intelligence data in a dashboard on the screen. Use this to show key metrics and charts to the user after you have performed an analysis.',
    parameters: {
      type: 'OBJECT',
      properties: {
        metrics: {
          type: 'ARRAY',
          description: 'An array of key metrics to display.',
          items: {
            type: 'OBJECT',
            properties: {
              label: { type: 'STRING' },
        ...
        charts: {
          type: 'ARRAY',
          description: 'An array of chart configurations to display.',
          items: {
            type: 'OBJECT',
            properties: {
              type: { type: 'STRING', enum: ['bar', 'doughnut', 'pie'] },
              title: { type: 'STRING' },
              data: { type: 'OBJECT' }, // Chart.js data object
              options: { type: 'OBJECT' }, // Chart.js options object
            },
