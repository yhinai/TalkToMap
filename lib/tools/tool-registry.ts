...
import { MapMarker, useDashboardStore, useLogStore, useMapStore } from '@/lib/state';
...
const displayDashboard: ToolImplementation = async (args, context) => {
  const { metrics, charts } = args;
  useDashboardStore.getState().setData({ metrics, charts });
  return `Dashboard updated with ${metrics?.length || 0} metrics and ${
    charts?.length || 0
  } charts.`;
};
